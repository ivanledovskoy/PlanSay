...
def get_task_by_id(id: int, db: Session = Depends(get_db), user = Depends(get_current_auth_user)):
    return _get_tasks_with_filter(db, user.user_id, [Task.id == id])


@router.delete("/tasks/{id}", summary="Удаление задачи пользователя")
def delete_task_by_id(id: int, db: Session = Depends(get_db), user = Depends(get_current_auth_user)):
...
def put_task_by_id(id: int, task_data: TaskUpdate, db: Session = Depends(get_db), user = Depends(get_current_auth_user)):
    with handle_db_exception(db):
